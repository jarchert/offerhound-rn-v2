...
interface ScoutProfileImageUploadProps {
  scoutId: string;
...
export function ScoutProfileImageUpload({
  scoutId,
  currentImageUrl,
  scoutName,
  onImageUpdated,
}: ScoutProfileImageUploadProps) {
  const [isUploading, setIsUploading] = useState(false);
  const [isDeleting, setIsDeleting] = useState(false);
  const { toast } = useToast();

  // Parity with Lovable: h-24 w-24 = 96px, icon h-10 w-10 = 40px
  const sizePx = 96;
  const iconPx = 40;
  ...
  const uploadImage = async (asset: ImagePicker.ImagePickerAsset) => {
    ...
    try {
      ...
      const fileName = `scouts/${scoutId}/profile-${Date.now()}.${fileExt}`;
  ...
  const fallbackChar = scoutName?.charAt(0);
...
export default ScoutProfileImageUpload;
