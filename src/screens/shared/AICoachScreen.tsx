...
// Coach avatar mirrors MAIN's GlobalAICoachIcon — the same 512x512 image the
// web app renders in the header, initial state, and per-message bubbles.
    ...
    <SafeAreaView style={s.container}>
      ...
      <View style={s.header}>
        ...
        <Image
          ...
          accessibilityLabel="OfferHound Coach avatar"
          testID="ai-coach-avatar"
        />
      ...
      <KeyboardAvoidingView
        ...
        {turns.length <= 1 ? (
          <View style={s.prompts} testID="ai-coach-prompts">
            {QUICK_PROMPTS.map((p) => (
              <Pressable
                key={p}
                style={s.prompt}
                ...
                accessibilityLabel={`Suggested question: ${p}`}
                testID={`ai-coach-prompt-${p}`}>
        ...
        <View style={s.inputRow}>
          <TextInput
            ...
            accessibilityLabel="Ask the AI coach"
          />
          <Pressable
            onPress={() => send()}
...
const s = StyleSheet.create({
  ...
  row: { alignItems: 'flex-start', marginVertical: 4, flexDirection: 'row' },
  rowMine: { alignItems: 'flex-end', justifyContent: 'flex-end' },
  bubble: {
    maxWidth: '88%',
