// src/__tests__/Bug10Fixes.test.tsx
// Bug 10 — AthleteSearchScreen: sport-match athletes sort to top; ALL athletes present
// Split from Bug89And10Fixes.test.tsx to isolate from cross-describe act() leaks.
...
// ═══════════════════════════════════════════════════════════════════════════════
// Bug 10 — AthleteSearchScreen sport-match sort to top (all athletes present)
// Namespace: 'bug10'
// ═══════════════════════════════════════════════════════════════════════════════
describe('Bug 10 — AthleteSearchScreen sport-match sort to top (all athletes present)', () => {
  const NS = 'bug10';

  const ATHLETES_WITH_SPORTS = [
    { id: 'a1', full_name: 'Alice Smith', position: 'QB', sport: 'Football', school: 'HSA', graduation_year: '2026', city: 'Austin', state: 'TX' },
    { id: 'a2', full_name: 'Bob Jones', position: 'WR', sport: 'Football', school: 'HSB', graduation_year: '2026', city: 'Dallas', state: 'TX' },
    { id: 'a3', full_name: 'Carol Davis', position: 'S', sport: 'Basketball', school: 'HSC', graduation_year: '2027', city: 'Houston', state: 'TX' },
    { id: 'a4', full_name: 'Dan Evans', position: 'CB', sport: 'Football', school: 'HSD', graduation_year: '2025', city: 'SF', state: 'CA' },
    { id: 'a5', full_name: 'Eve Wilson', position: 'PG', sport: 'Basketball', school: 'HSE', graduation_year: '2026', city: 'Denver', state: 'CO' },
  ];

  beforeEach(() => {
    _clearNamespace(NS);
    // AthleteSearchScreen: useQuery({ queryKey: ['athlete-search', search, position, gradYear] })
    // _getQueryKey extracts 'athlete-search' as first array element.
    _register(NS, 'athlete-search', ATHLETES_WITH_SPORTS);
  });

  afterEach(() => { _clearNamespace(NS); });

  it('sport-matching athletes appear with "Your sport" label', async () => {
    const AthleteSearch = require('@/screens/shared/AthleteSearchScreen').default;
    const qc = makeQueryClient();
    const wrapper = ({ children }: any) => (
      <QueryClientProvider client={qc}>{children}</QueryClientProvider>
    );
    const { getAllByText } = await render(<AthleteSearch />, { wrapper });
    await act(async () => {});
    await waitFor(() => { expect(getAllByText('Your sport').length).toBe(3); });
  });

  it('ALL 5 athletes are still rendered (no hard exclude)', async () => {
    const AthleteSearch = require('@/screens/shared/AthleteSearchScreen').default;
    const qc = makeQueryClient();
    const wrapper = ({ children }: any) => (
      <QueryClientProvider client={qc}>{children}</QueryClientProvider>
    ...
    await act(async () => {});
    await waitFor(() => {
      expect(getAllByText('Carol Davis').length).toBeGreaterThan(0);
      expect(getAllByText('Eve Wilson').length).toBeGreaterThan(0);
    });
    const names = ['Alice Smith', 'Bob Jones', 'Carol Davis', 'Dan Evans', 'Eve Wilson'];
    for (const n of names) {
      expect(getAllByText(n).length).toBeGreaterThan(0);
    }
  });

  it('non-matching athletes do NOT get "Your sport" label', async () => {
    const AthleteSearch = require('@/screens/shared/AthleteSearchScreen').default;
    const qc = makeQueryClient();
    const wrapper = ({ children }: any) => (
      <QueryClientProvider client={qc}>{children}</QueryClientProvider>
    );
    const { getAllByText, queryAllByText } = await render(<AthleteSearch />, { wrapper });
    await act(async () => {});
    await waitFor(() => { expect(queryAllByText('Your sport').length).toBe(3); });
    expect(getAllByText('Carol Davis').length).toBeGreaterThan(0);
    expect(getAllByText('Eve Wilson').length).toBeGreaterThan(0);
  });

  it('when coach has no sport, no athlete gets "Your sport" label', async () => {
    mockCoachProfile.mockReturnValue({
    ...
    const { getAllByText, queryAllByText } = await render(<AthleteSearch />, { wrapper });
    await act(async () => {});
    await waitFor(() => { expect(queryAllByText('Your sport').length).toBe(0); });
    expect(getAllByText('Alice Smith').length).toBeGreaterThan(0);
    expect(getAllByText('Carol Davis').length).toBeGreaterThan(0);
  });

  it('HS coach sport is also respected (Basketball coach sees Basketball athletes)', async () => {
    mockCoachProfile.mockReturnValue({ data: null, isLoading: false, isFetched: true });
    mockHSCoachProfile.mockReturnValue({ data: { id: 'hs-1', sport: 'Basketball' } });
    const AthleteSearch = require('@/screens/shared/AthleteSearchScreen').default;
    const qc = makeQueryClient();
    const wrapper = ({ children }: any) => (
      <QueryClientProvider client={qc}>{children}</QueryClientProvider>
    ...
    const { getAllByText } = await render(<AthleteSearch />, { wrapper });
    await act(async () => {});
    await waitFor(() => { expect(getAllByText('Your sport').length).toBe(2); });
    expect(getAllByText('Alice Smith').length).toBeGreaterThan(0);
