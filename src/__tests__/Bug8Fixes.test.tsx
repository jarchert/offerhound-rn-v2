// src/__tests__/Bug8Fixes.test.tsx
...
// ═══════════════════════════════════════════════════════════════════════════════
// Bug 8 — ClubCoachDashboardScreen: OrganizationLogoUpload wired in profile tab
// Namespace: 'bug8'
// ═══════════════════════════════════════════════════════════════════════════════
describe('Bug 8 — ClubCoachDashboardScreen OrganizationLogoUpload wiring', () => {
  const NS = 'bug8';
  ...
  beforeEach(() => {
    _clearNamespace(NS);
    _register(NS, 'club-coach-profile-full', CLUB_PROFILE);
    _register(NS, 'club-teams', []);
    _register(NS, 'club-contacts-count', 0);
    _register(NS, 'club-media-count', 0);
    _register(NS, 'club-conversion-insight', { unsubscribed: 0, lockedContacts: 0 });
  });

  afterEach(() => { _clearNamespace(NS); });

  it('OrganizationLogoUpload renders in ClubCoachDashboard profile tab', async () => {
    const ClubCoach = require('@/screens/club/ClubCoachDashboardScreen').default;
    const qc = makeQueryClient();
    const wrapper = ({ children }: any) => (
      <QueryClientProvider client={qc}>{children}</QueryClientProvider>
    );
    // IMPORTANT: await render() — RTL v14 returns a thenable.
    const { queryByText } = await render(<ClubCoach />, { wrapper });
    await act(async () => {});
    expect(queryByText('Upload Logo') || queryByText('Change Logo')).toBeTruthy();
  });

  it('OrganizationLogoUpload shows "Change Logo" when club_logo_url is set', async () => {
    _clearNamespace(NS);
    const withLogo = { ...CLUB_PROFILE, club_logo_url: 'https://example.com/logo.png' };
    _register(NS, 'club-coach-profile-full', withLogo);
    _register(NS, 'club-teams', []);
    _register(NS, 'club-contacts-count', 0);
    _register(NS, 'club-media-count', 0);
    _register(NS, 'club-conversion-insight', { unsubscribed: 0, lockedContacts: 0 });
    ...
    const { getByTestId, getByText } = await render(<ClubCoach />, { wrapper });
    await act(async () => {});
    await act(async () => { fireEvent.press(getByTestId('tab-trigger-profile')); });
    expect(getByText('Change Logo')).toBeTruthy();
  });

  it('Upload Logo button fires image picker without crashing', async () => {
    _clearNamespace(NS);
    _register(NS, 'club-coach-profile-full', CLUB_PROFILE);
    _register(NS, 'club-teams', []);
    _register(NS, 'club-contacts-count', 0);
    ...
    await act(async () => {});
    await act(async () => { fireEvent.press(getByTestId('tab-trigger-profile')); });
    const uploadBtn = getByText('Upload Logo');
